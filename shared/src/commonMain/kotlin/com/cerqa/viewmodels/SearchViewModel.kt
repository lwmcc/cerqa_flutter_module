...
class SearchViewModel(
    ...
    /**
     * Create test user 1 (Larry McCarty) - Temporary for testing
     * Call this when logged in as the user you want to add to the backend
     */
    fun createTestUser1() {
        println("SearchViewModel: createTestUser1 called")
        scope.launch {
            _uiState.value = _uiState.value.copy(pending = true)

            repository.createCurrentUserWithDetails(
                firstName = "Larry",
                lastName = "McCarty",
                name = "LM",
                phone = "+14808104545",
                ...
                println("SearchViewModel: User created successfully")
                _uiState.value = _uiState.value.copy(
                    pending = false,
                    message = MessageType.SUCCESS
                )
            }.onFailure { error ->
                println("SearchViewModel: User creation failed: ${error.message}")
                error.printStackTrace()
                _uiState.value = _uiState.value.copy(
                    pending = false,
                    message = MessageType.ERROR
    ...
    /**
     * Create test user 2 (LeBron James) - Temporary for testing
     * Call this when logged in as the user you want to add to the backend
     */
    fun createTestUser2() {
        scope.launch {
